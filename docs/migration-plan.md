...
| 4. Multi-harness observation ([WP #784](https://openproject.tail21f530.ts.net/work_packages/784)) | Concurrent parent/subagent threads, recent completions, attention signals, freshness, and native observation links work across supported harnesses. | Codex is live in UAT. The additive event API and local Cursor/OhMyPi reporters are installed; real CLI turns from both harnesses reached UAT as completed with measured durations and unassessed outcomes. Cursor desktop Agent Chat, subagent lifecycle, native task URLs, cloud enrollment, and reliable child-stop correlation remain unverified. |
